...
<div class="grid_16 mt10">
		...
		<div id="playerBioInfoCardMain">
			...
			<h3 class="title-grey">${ usuarioInstance?.persona?.nombre + " " + usuarioInstance?.persona?.apellido}</h3>
				<div id="playerBioHeadShot">
					<p class="IE6pngFix" id="playerBioHeadShotCopy">&copy; ACS</p>					
					<img width="140px" height="200px"
			...
			<ul id="playerBioInfoList">				
				...
				<g:if test="${ usuarioInstance?.persona?.domicilio }">
					<li>
						<span>Residencia: </span> ${ usuarioInstance?.persona?.domicilio?.toString()} 
					</li>
				</g:if>				
				...
				<g:if test="${ categoriaSeleccionada}">
					<li><span>Categoria: </span>${ categoriaSeleccionada?.toString() }</li>
				</g:if>
			</ul>			
			<div id="playerBioInfoExtra">
			<div id="playerBioInfoRank">
				<g:if test="${usuarioInstance?.jugador?.rankings?.size()>0}">
	...
	<g:if test="${ !torneos }">
			<h2 class="mt20">${usuarioInstance?.persona?.nombre} no ha participado en torneos el ${year }</h2>
	</g:if>
